...
public class HorusManager : MonoBehaviour {
	...
	public Vector2Int screenSzie = new Vector2Int( 640,480 );

	// Use this for initialization
	void Start () {
		ResetPlugin();
		cameraInst = gameObject.GetComponent<Camera>();
		SetBufferSize(screenSzie.x, screenSzie.y);
		PopulateRenderers();
	...
	void Update () {
		UnityEngine.Profiling.Profiler.BeginSample("__UpdateCameraSettings__");
		UpdateCameraSettings();
		UnityEngine.Profiling.Profiler.EndSample();
		UnityEngine.Profiling.Profiler.BeginSample("__CalculateVisibility__");
		CalculateVisibility();
		UnityEngine.Profiling.Profiler.EndSample();
	}
	...
	{	
		UnityEngine.Profiling.Profiler.BeginSample("__MarshallVisibilityResults__");
		var visibileIndices = MarshallVisibilityResults();
		UnityEngine.Profiling.Profiler.EndSample();
		if (visibileIndices == null)
        {	
			int length = staticRenderers.Count;
            for (int i = 0; i < length; i++)
            {
				var v = staticRenderers[i];
				v.enabled = false;
			}	
   ...
   //         }
		...
		int total = staticRenderers.Count;
		for (int t = 0; t < total; t++)
        {	
			var v = staticRenderers[t];
			if (isVisible(visibileIndices, t)) 
				v.enabled = true;
			else 
				v.enabled = false;
        }	
